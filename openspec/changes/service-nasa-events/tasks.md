...
- [ ] 1.2 Include at least one concrete JSON example for `GET /api/events` and one for `GET /health` in the convention document.
...
- [ ] 3.4 Add `GET /api/events` with the parameterless JSON contract defined in the convention document.

## 4. Verification

- [ ] 4.1 Add unit tests for the normalization logic and the upstream adapter error handling.
- [ ] 4.2 Add E2E tests covering service startup, `GET /health`, and `GET /api/events`.
